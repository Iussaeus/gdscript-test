@tool
class_name IsLaserCollidingBlock extends CodeBlock
...
func _ready() -> void:
	...
	set_block_name("IsLaserCollidingBlock")
...
func _logic(_params: Array=[]) -> Variant:
	...
	var result := GameState.laser.is_colliding()
	print("INFO: %s _logic called returning - %s from %s" % [name, result])
	return result

func get_state() -> Dictionary:
	return {
		type = IsLaserCollidingBlock,
