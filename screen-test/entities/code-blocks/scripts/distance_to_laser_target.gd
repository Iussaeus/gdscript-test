@tool
class_name DistanceToLaserTargetBlock extends CodeBlock
...
func _ready() -> void:
	...
	set_block_name("DistanceToLaserTargetBlock")
...
func _logic(_params: Array=[]) -> Variant:
	...
	var result := await GameState.laser.distance_to_target()
	print("INFO: %s _logic called returning- %s" % [name, result])
	return result

func get_state() -> Dictionary:
	return {
		type = DistanceToLaserTargetBlock,
