@tool
class_name ArithmeticBlock extends CodeBlock
...
func _ready() -> void:
	...
	in_connections = 3
	out_connections = 2
	set_pin_names(["lhs", "rhs"], ["result"])

	set_block_name("ArithmeticBlock")
...
func _item_selected(idx: int) -> void:
	...
	print("INFO: ArithmeticBlock selected operation - %s" % operation)
