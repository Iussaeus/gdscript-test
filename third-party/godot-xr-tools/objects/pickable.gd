...
# Signal emitted when this object is hit by function_pointer
...
@export var bonus_velocity : float = 1
...
func _ready():
	...
	linear_velocity *= bonus_velocity
	angular_velocity *= bonus_velocity
	# Get all grab points
...
func controller_action_release(controller : XRController3D):
	...
	if (
		is_instance_valid(_grab_driver.primary) and _grab_driver.primary.point
		and _grab_driver.primary.controller == controller
	):
