...
const _DefaultMaterial := preload("res://third-party/godot-xr-tools/materials/capsule.tres")
...
@export_range(-1, 0, 0.1) var y_min: float = -0.1
...
## Teleport texture
@export
var arc_texture: Texture2D = preload("res://third-party/godot-xr-tools/images/teleport_arrow.png"):
	set = set_arc_texture

## Target texture
@export
var target_texture: Texture2D = preload("res://third-party/godot-xr-tools/images/teleport_target.png"):
...
## [XRController3D] node.
@onready var camera := XRHelpers.get_xr_camera(self)
...
func _physics_process(delta):
	...
	if controller and controller.get_is_active() and is_input_valid():
		if !is_teleporting:
			is_teleporting = true
		...
		if hit_something:
			...
			target_basis = target_basis.rotated(normal, 0)
		...
		if can_teleport:
			...
			# Teleport the player
			# player_body.rotation_degrees = camera.rotation_degrees
...
func is_input_valid():
	var input_type := typeof(controller.get_input(teleport_button_action))
	var input := controller.get_input(teleport_button_action)
	match input_type:
		# x < 0.3 and x > -0.3
		# y < - 0.2
		TYPE_VECTOR2:
			var vec_input := input as Vector2
			if vec_input.x < x_max and vec_input.x > x_min and vec_input.y < y_min:
				return true
			return false
		TYPE_BOOL:
