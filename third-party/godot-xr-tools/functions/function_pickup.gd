@tool
@icon("res://third-party/godot-xr-tools/editor/icons/function.svg")
...
## Ranged pickup controller axis
@export var ranged_pickup_axis_action : String = "trigger_click"

## Action controller button
@export var primary_action_button_action : String = "trigger_click"
## Action controller button
@export var secondary_action_button_action : String = "by_button"
## Action controller button
@export var ternary_action_button_action : String = "xa_button"
...
var _start_function_pointer_rot: Vector3
...
@onready var _origin := XRHelpers.get_xr_origin(self)

@onready var _function_pointer : XRToolsFunctionPointer = _controller.get_node("FunctionPointer")
...
func _on_ranged_exited(target: Node3D) -> void:
	if target is XRToolsPickable and _object_in_ranged_area.size() == 1:
		focused_object = null
		_function_pointer.basis = _function_pointer.base_basis
	
	if _object_in_ranged_area.size() > 1:
		focused_object = _get_closest_ranged()
...
func _update_closest_object() -> void:
	...
	if not picked_up_object:
		...
		# if not new_closest_obj and ranged_enable and _controller.get_input(ranged_pickup_axis_action) \
		# 	and _function_pointer.target:
		if not new_closest_obj and ranged_enable and _controller.get_input(ranged_pickup_axis_action):

			# Find closest in ranged area
			if is_instance_valid(focused_object):
				new_closest_obj = focused_object
			else:
...
func _pick_up_object(target: Node3D) -> void:
	...
	var is_ranged_target := _object_in_ranged_area.has(target)
	var is_ranged_grab_pressed:bool = get_controller().get_input(ranged_pickup_axis_action)
...
func _on_button_pressed(p_button) -> void:
	if p_button == primary_action_button_action and is_instance_valid(picked_up_object):
