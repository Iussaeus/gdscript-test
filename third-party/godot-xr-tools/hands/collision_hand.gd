...
func _ready():
	...
	if not palm_collision:
		...
		_palm_collision_shape.shape = \
			preload("res://third-party/godot-xr-tools/hands/scenes/collision/hand_palm.shape")
		...
		if bone_name == "Palm_L":
			...
			if _digit_collision_shapes.has(bone_name):
				...
				collision_node.shape = \
					preload("res://third-party/godot-xr-tools/hands/scenes/collision/hand_digit.shape")
